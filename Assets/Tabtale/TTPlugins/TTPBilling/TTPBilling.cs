...
namespace Tabtale.TTPlugins
{
    ...
    public class TTPBilling
    {
                ...
                (PurchaseIAPResult purchaseIAPResult) =>
                {
                    ...
                    if (Impl is IosImpl)
                    {
                        if(purchaseIAPResult.result == PurchaseIAPResultCode.Success)
                            ((IosImpl)Impl).ReportPurchaseToConversion(purchaseIAPResult.purchasedItem.localizedPrice, purchaseIAPResult.purchasedItem.mainId, IsConsumable(purchaseIAPResult.purchasedItem.mainId));
                        else
        ...
        static void InternalInit()
        {
            ...
            {
                if (!Configuration.GetBool("deferInit"))
                {
        ...
        private static Dictionary<string,object> Configuration
        {
            get
            {
                if (_configuration == null)
                {
                    ...
                    {
                        _configuration = Json.Deserialize(configurationJson) as Dictionary<string,object>;
                    }
                }
        ...
        private class IosImpl : ITTPBilling
        {
            ...
            public void ReportPurchaseToConversion(string price, string productId, bool consumable)
            {
                var consumableStr = consumable ? "true" : "false";
                var message = "{\"price\":"+price+",\"productId\":\""+productId+"\",\"consumable\":"+consumableStr+"}";
