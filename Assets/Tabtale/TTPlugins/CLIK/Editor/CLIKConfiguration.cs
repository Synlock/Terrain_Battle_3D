...
public class CLIKConfiguration : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        IndicateConfiguration("Billing", _configuration.billingConfig.IsValid, _configuration.billingConfig.included);

        if (_configuration.billingConfig.included)
        {
            if (!IsUnityPurchasingEnabled())
            {
                GUILayout.Label("Billing is included but Unity Purchasing IAP is not enabled. Please enable it in Services Tab. (Do NOT Reimport)", _redLabel);
            }
            showInAppsSection = EditorGUILayout.Foldout(showInAppsSection, "Iaps");
            if (showInAppsSection)
            {
                EditorGUILayout.Separator();
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.BeginVertical();
                newInAppId = EditorGUILayout.TextField("InApp id: ", newInAppId);
                newInAppIapId = EditorGUILayout.TextField("InApp iapId: ", newInAppIapId);
                EditorGUILayout.EndVertical();
                EditorGUILayout.BeginVertical();
                newInAppType = EditorGUILayout.Popup("InApp Type", newInAppType, BillingConfig.InAppTypes);
                newInAppNoAds = EditorGUILayout.Toggle("No Ads", newInAppNoAds);
                EditorGUILayout.EndVertical();
                EditorGUILayout.EndHorizontal();
                if (GUILayout.Button("Add InApp"))
                {
                    Debug.Log("Add InApp id = " + newInAppId + " iapId = " + newInAppIapId);
                    if (string.IsNullOrWhiteSpace(newInAppId) || string.IsNullOrWhiteSpace(newInAppIapId))
                    {
                        Debug.LogError("id or iapId are empty");
                    }
                    else
                    {
                        Dictionary<string, object> inAppDict = new Dictionary<string, object>();
                        inAppDict["id"] = newInAppId.Trim();
                        inAppDict["iapId"] = newInAppIapId.Trim();
                        inAppDict["type"] = BillingConfig.InAppTypes[newInAppType];
                        inAppDict["noAds"] = newInAppNoAds;
                        _configuration.billingConfig.AddInApp(inAppDict);
                        newInAppId = "";
                        newInAppIapId = "";
                        newInAppType = 0;
                        newInAppNoAds = false;
                ...
                EditorGUILayout.Separator();
                foreach (Dictionary<string, object> inAppDict in _configuration.billingConfig.GetInApps())
                {
                    object deleted = false;
                    if (inAppDict.TryGetValue("deleted", out deleted))
                    {
                        if ((bool)deleted)
                        {
                            continue;
                    ...
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.BeginVertical();
                    inAppDict["id"] = EditorGUILayout.TextField("InApp id: ", (string)inAppDict["id"]);
                    inAppDict["iapId"] = EditorGUILayout.TextField("InApp iapId: ", (string)inAppDict["iapId"]);
                    EditorGUILayout.EndVertical();
                    EditorGUILayout.BeginVertical();
                    int pos = EditorGUILayout.Popup("InApp Type", Array.IndexOf(BillingConfig.InAppTypes, (string)inAppDict["type"]), BillingConfig.InAppTypes);
                    inAppDict["type"] = BillingConfig.InAppTypes[pos];
                    inAppDict["noAds"] = EditorGUILayout.Toggle("No Ads", (bool)inAppDict["noAds"]);
                    EditorGUILayout.EndVertical();
                    if (GUILayout.Button(_minusTexture, _minusIcon))
                    {
                        Debug.Log("Remove inApp " + (string)inAppDict["id"]);
                        _configuration.billingConfig.RemoveInApp((string)inAppDict["id"]);
                    }
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.Separator();
                    GuiLine(1);
                ...
                EditorGUILayout.Separator();
                if (GUILayout.Button("Save Billing Configuration"))
                {
                    Debug.Log("Save Billing Configuration");
                    _configuration.SaveBillingConfiguration();
                }
            }
        }
    ...
    private class BillingConfig : IConfig
    {
        ...
        private Dictionary<string, object> _billingConfig;

        public static string[] InAppTypes = new string[3] { "subscription", "consumable", "non-consumable" };

        private List<Dictionary<string, object>> listInApps = new List<Dictionary<string, object>>();
        public List<Dictionary<string, object>> GetInApps()
        {
            return listInApps;
        }
        ...
        public void AddInApp(Dictionary<string, object> inApp)
        {
            listInApps.Add(inApp);
        }

        public void RemoveInApp(string id)
        {
            Debug.Log("BillingConfig::RemoveInApp:id=" + id);
            foreach (Dictionary<string, object> inApp in listInApps)
            {
                object value = "";
                if (inApp.TryGetValue("id", out value))
                {
                    if (id.Equals((string)value))
                    {
                        inApp["deleted"] = true;
                        break;
                    }
        ...
        public void ParseListInApps()
        {
            listInApps.Clear();
            object iaps;
            if (_billingConfig.TryGetValue("iaps", out iaps))
            {
                List<object> listObjects = (List<object>)iaps;
                foreach (object inAppObject in listObjects)
                {
                    if (inAppObject.GetType() == typeof(Dictionary<string, object>))
                    {
                        Dictionary<string, object> inAppDict = inAppObject as Dictionary<string, object>;
                        //InApp inApp = JsonUtility.FromJson<InApp>(TTPJson.Serialize(inAppDict));
                        listInApps.Add(inAppDict);
                    }
                }
            }
        }

        public void SetInAppsToDict()
        {
            List<Dictionary<string, object>> jsonListInApps = new List<Dictionary<string, object>>();
            foreach (Dictionary<string, object> inAppDict in listInApps)
            {
                object deleted = false;
                if (inAppDict.TryGetValue("deleted", out deleted))
                {
                ...
                jsonListInApps.Add(inAppDict);
            }
            _billingConfig["iaps"] = jsonListInApps.ToArray();
        }
        ...
        public void LoadFromFile()
        {
            ...
            if (File.Exists(fp))
            {
                ...
                if (!string.IsNullOrEmpty(json))
                {
                    var cfg = TTPJson.Deserialize(json) as Dictionary<string, object>;
                    if (cfg != null)
                    {
                        _billingConfig = cfg;
                        if (_billingConfig.ContainsKey(Constants.CONFIG_KEY_INCLUDED) &&
                            _billingConfig[Constants.CONFIG_KEY_INCLUDED] is bool includedVal)
                        {
                    ...
                    {
                        _billingConfig = new Dictionary<string, object>();
                    }
                    ParseListInApps();
                }
            }
        ...
        public bool IsValid
        {
            get { return _isValid && IsUnityPurchasingEnabled(); }
        }
    }
    ...
    private class PlatformConfiguration
    {
        ...
        public void SaveBillingConfiguration()
        {
            billingConfig.SetInAppsToDict();
            SaveConfigurationToFile(billingConfig);
        }
